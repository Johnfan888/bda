...
a2 = pd.read_csv(r'E:\workspace\test\recommend\data\a3.csv')

a2 = np.array(a2)
s={}
for i in set(train['user_id'].values):
    e = train[train['user_id'] == i]
    b=0
    for x,y,z in e.values:
        if a2[int(x-1)][int(y-1)]-z < 0.1:
            b += 1
    s[i] = b/len(e)
# 测试集与专家的余弦相似度    
l1 = dict()
...
#         l1[i][j] = (2*cosine(i,j)*s[j])/(cosine(i,j)+s[j])
        if calcPearson(i,j)>0:
            l1[i][j] = (2*calcPearson(i,j)*s[j])/(calcPearson(i,j)+s[j])
        else:
            l1[i][j]=0
