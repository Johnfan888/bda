...
for i in range(32,len(train_df)):
...
def mymodel(train_data):
    ...
    X_train = train_data.drop([ 'count1','1day_max','2day_max',
                                # '3day_max',
                                '4day_max','7day_max','14day_max','21day_max',
                                '30day_max'],axis=1)
    y_train = train_data.loc[:,'count1']
...
for j in range(0, len(test_df)):
    ...
    b = np.delete(a, [4, 5, 7, 8, 9, 10, 11], axis=1)
    c = round(abs(my_model.predict(b)))
    y_pre.append(c)
...
print res
res.to_csv('/root/word2vec/yancheng/data/pre/GBR23.csv',index=None,header=None,sep='\t')