...
# 数据：去除前7天的
# 模型：GBR
# 特征：date,day_of_week,n_time,day_std,'1day_mean','2day_mean','3day_mean','4day_mean','7day_mean'
# 线下测试：631828.839168 ，520426.387614，2095978.61486
...
for i in range(9,len(all_data_1)):
    frame_final = feature_1(i)
    frame_boss = pd.concat([frame_boss,frame_final],axis=1)
frame_boss =  (frame_boss.T).reset_index(drop=True)


train_df1 = frame_boss.iloc[:820]
test_df1 = frame_boss.iloc[820:,:]

train_df2 = frame_boss.iloc[:769]
test_df2 = frame_boss.iloc[769:,:]

train_df3 = frame_boss.iloc[:683]
test_df3 = frame_boss.iloc[683:,:]


def validation(train_df,test_df):
    # # 切分数据（训练集和测试集）

    X_train = train_df[['date','day_of_week','n_time','day_std','1day_mean','2day_mean','3day_mean',
                        '4day_mean','7day_mean']].values
    y_train = train_df['count1'].values
    X_test = test_df[['date', 'day_of_week','n_time','day_std','1day_mean','2day_mean','3day_mean',
                        '4day_mean','7day_mean']].values
